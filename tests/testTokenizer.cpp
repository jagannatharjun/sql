...
inline sql::tokens getAllTokens(sql::string str) { return sql::getAllTokens(std::move(str)); }
...
TEST(testTokenizer, tokensWithSymbols) {
  ...
  expect_token({"Values", TokenType::Function});
...
TEST(testTokenizer, testFunctionFinding) {
  auto t = sql::tokenStream("Values('Hello')");
  auto f = sql::function_token(t);
  ASSERT_EQ(t.current().first, ")");
  ASSERT_EQ(f.Args.size(), 1);
  ASSERT_EQ(f.Args.front().front().first, "Hello");
  ASSERT_EQ(f.Name, "Values");

  t = sql::tokenStream("Values('Hello' 'lol',int Uname Primary Key)");
  f = sql::function_token(t);
  ASSERT_EQ(t.current().first, ")");
  ASSERT_EQ(f.Args.size(), 2);
  using TokenType = sql::tokenizer::TokenType;
  sql::tokens firstArgument = {{"Hello", TokenType::StringLiteral},
                               {"lol", TokenType::StringLiteral}};
  ASSERT_EQ(f.Args[0], firstArgument);
  sql::tokens secondArgument = {{"int", TokenType::Identifier},
                                {"uname", TokenType::Identifier},
